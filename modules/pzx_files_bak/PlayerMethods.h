...
namespace LuaPlayer
{
	...
	int AddItemByID(lua_State* L, Player* player)//新增接口通过ID获取物品
	{
		...
		if (temp) {
            ...
            std::vector<std::string>::iterator itor = allForbi.begin();
    ...
    int createCusInstance(lua_State* L, Player* player)
    {
        ...
        std::regex re{ "," };
        std::vector<std::string> creatureVector;
        std::vector<std::string> gobjectVector;
        ...
        try
        {
            ...
            stmt->SetData(0, newInstanceId);
            stmt->SetData(1, mapid);
            stmt->SetData(2, uint32(respawnTime));
            stmt->SetData(3, uint8(Difficulty(diffc)));
            stmt->SetData(4, 0);
            stmt->SetData(5, data);
            CharacterDatabase.Execute(stmt);

            creatureVector = std::vector<std::string>{ std::sregex_token_iterator(creatureString.begin(), creatureString.end(), re, -1),std::sregex_token_iterator() };
            gobjectVector = std::vector<std::string>{ std::sregex_token_iterator(gobjectString.begin(), gobjectString.end(), re, -1),std::sregex_token_iterator() };
            //构造sql语句
            for (int i = 0; i < creatureVector.size(); ++i) {
                if (creatureVector[i].empty() || creatureVector[i].at(0) == ' ')
                    continue;
                int dbGuid = std::stoi(creatureVector[i]);
                auto* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CREATURE_RESPAWN);
                stmt->SetData(0, dbGuid);
                stmt->SetData(1, uint32(respawnTime));
                stmt->SetData(2, mapid);
                stmt->SetData(3, newInstanceId);
                CharacterDatabase.Execute(stmt);
            }
        ...
        sInstanceSaveMgr->PlayerCreateBoundInstancesMaps(player->GetGUID());//非常重要
